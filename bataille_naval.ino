...
EspMQTTClient client(
  ...
  "ESP32_C",   // Can be omitted if not needed
  "clientId-qjRLv5CeTd",    // Client name that uniquely identify your device
  1883 // The MQTT port
);*/
   ...
   for (int i = 0; i<5; i++){
    if (valeurnuke == 0){
      if (i == a){
        ...
        if (a >= 4){
          if ((bat[i][1]-1 <= pointY && pointY <= bat[i][3]+1) || ((bat[i][1]-1 <= pointY+2) && (pointY+2 <= bat[i][3]+1)) || (bat[i][1] <= pointY+bateaux[a] && pointY+bateaux[a] <= bat[i][3]+1)){
          Serial.println("X ckecked");
            if (bat[i][0]-1 <= pointX && pointX <= bat[i][2]+1){
              Serial.print("i:");
              Serial.println(i);
              ...
              Serial.print("i:");
              Serial.println(i);
              Serial.println("Placement possible 1000");
              m++;
              //placement possible
            }
            ...
            Serial.print("i:");
            Serial.println(i);
            Serial.println("Placement possible 2000");
            m++;
            //placement possible
            }
        }else if ((bat[i][0]-1 <= pointX && pointX <= bat[i][2]+1) || (bat[i][0] <= pointX+bateaux[a] && pointX+bateaux[a] <= bat[i][2]+1)){
          Serial.println("X ckecked");
          if (bat[i][1]-1 <= pointY && pointY <= bat[i][3]+1){
            Serial.print("i:");
            Serial.println(i);
            Serial.println("Placement impossible");
            //placement impossible
      ...
      if (i == a){
        ...
        if (a >= 4){
          ...
          if ((bat[i][1]-1 <= pointY && pointY <= bat[i][3]+1) || (bat[i][1] <= pointY+bateaux[a] && pointY+bateaux[a] <= bat[i][3]+1)){
          Serial.println("Y ckecked");
          if (bat[i][0]-1 <= pointX && pointX <= bat[i][2]+1){
            Serial.print("i:");
            Serial.println(i);
    ...
    int a = 0;
       while(a<100){
          ...
          if (valeurbouton == 0){
                  ...
                  friendlyFire[pointX-11][pointY-11]=1;
                  Serial.print("attaque:");
                  Serial.print(pointX-11);Serial.println(pointY-11);
                  Serial.println(friendlyFire[pointX-11][pointY-11]);
